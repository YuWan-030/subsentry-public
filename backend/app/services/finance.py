...
def list_financial_logs(
    ...
    *,
    ...
    if clean_keyword:
        clauses.append("(customer_name LIKE ? OR renew_price LIKE ? OR remote_email LIKE ? OR owner_username LIKE ?)")
    ...
    total_rows = query(settings, f"SELECT COUNT(1) AS cnt FROM financial_logs {where_sql}", tuple(params))
    total = int(total_rows[0].get("cnt") or 0) if total_rows else 0
    total_amount_rows = query(settings, f"SELECT COALESCE(SUM(amount), 0) AS total_amount FROM financial_logs {where_sql}", tuple(params))
    total_amount = round(float(total_amount_rows[0].get("total_amount") or 0), 2) if total_amount_rows else 0.0
    rows = query(
        settings,
        f"""
        SELECT id, customer_id, owner_username, node_id, remote_email, customer_name, renew_price, amount, renew_days, new_expiry, created_at
        FROM financial_logs
        {where_sql}
        ORDER BY id DESC
        LIMIT ? OFFSET ?
    ...
    return {"items": rows, "total": total, "page": page, "per_page": per_page, "total_amount": total_amount}
